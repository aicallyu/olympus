...
import type { CodeCommitPayload, ExecutionQueueItem, FileAction } from './types.js';
 ...
 */
function isPathSafe(filePath: string): ValidationResult {
  // Normalize path
 ...
 */
function validateFile(file: FileAction): ValidationResult {
  // Check path safety
 ...
 */
function validateCodeCommit(payload: CodeCommitPayload): ValidationResult {
 ...
 */
export function validateExecution(item: ExecutionQueueItem): ValidationResult {
  ...
  switch (item.execution_type) {
    case 'code_commit':
      return validateCodeCommit(item.payload as CodeCommitPayload);
